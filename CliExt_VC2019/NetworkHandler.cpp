	...
	if(currentTick > g_NetworkHandler.m_TickTimeout)
	{
		...
		if(g_NetworkHandler.m_Initialized == FALSE)
		{
			...
			if (m_Enable)
			{
				...
				g_NetworkHandler.Bind(18, AAAAArray);
...
void __thiscall CNetworkHandler::OnConnect(PVOID pInstance, const char* sAddress, UINT port)
{
	...
	wchar_t ipBuffer[256] = { 0 };

	// Leer la IP desde el archivo INI
	GetPrivateProfileStringW(section, L"AuthIP", L"200.235.99.68", ipBuffer, 256, configFile);

	// Convertir wchar_t* a char*
	char ip[256];
	WideCharToMultiByte(CP_ACP, 0, ipBuffer, -1, ip, 256, NULL, NULL);

	// Usar la IP convertida en stringstream
	std::stringstream ipStream;
	ipStream << ip;

	// Llamar a la función original con la IP obtenida
...
int AAAAArray(NHandler *pNetwork, CInPacket& packet)
{
